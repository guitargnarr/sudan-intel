...
class BaseIngester(ABC):
    ...
    async def safe_fetch(self, db: AsyncSession) -> dict:
        try:
            ...
            await self._update_status(db, success=True, count=count)
            return {"source": self.source_name, "status": "ok", "records": count}
        except httpx.HTTPStatusError as e:
            error_msg = f"HTTP {e.response.status_code}: {e.response.text[:200]}"
            logger.warning("%s: %s", self.source_name, error_msg)
            try:
                await db.rollback()
                await self._update_status(db, success=False, error=error_msg)
            except Exception:
                pass
            return {"source": self.source_name, "status": "error", "error": error_msg}
        except Exception as e:
            error_msg = str(e)[:500]
            logger.error("%s: %s", self.source_name, error_msg)
            try:
                await db.rollback()
                await self._update_status(db, success=False, error=error_msg)
            except Exception:
    ...
    async def _update_status(self, db: AsyncSession, success: bool, count: int = 0, error: str = None):
        ...
        if success:
            ...
            status.total_records = (status.total_records or 0) + count
